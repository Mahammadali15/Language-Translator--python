...
#Title of the app
Label(window, text="PythonGeeks Language Translator Using Python", font=("Gathering", 12)).pack()
#Read inputs
#Text input
Label(window, text="Text to translate:").place(x=10, y=20)
text_entry = Text(window, width=40, height=5,font=("Ubuntu Mono",12))
text_entry.place(x=130,y=20)
#Source language input
Label(window, text="Source language (empty: auto-detect):").place(x=10, y=120)
src_entry = Text(window, width=20,height=1,font=("Ubuntu Mono",12))
src_entry.place(x=275,y=120)
#Destination input
Label(window, text="Target language (empty: english-default):").place(x=10, y=150)
...
Button(window, text='Translate', bg='Turquoise', command=translate_function).place(x=160, y=190)
Button(window, text='Clear', bg='Turquoise', command=clear).place(x=270, y=190)
#close the app
window.mainloop()
